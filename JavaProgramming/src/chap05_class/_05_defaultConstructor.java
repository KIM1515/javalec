...
import chap05_class_example.Student;
...
public class _05_defaultConstructor {
	...
	public static void main(String[] args) {
		...
		System.out.println("강의 학점는 : " + course.getcScore());
		
		System.out.println("--------------------------------------");
		
		//다른 패키지의 클래스는 임포트된 상태에서 사용
		//단축기 : ctrl+shift+o(영문자)
		//같은 2개 이상 생성하면 각각 다른 heap메모리 공간에 각각 생성
		//같은 객체라고 같은 메모리 공간의 하나의 객체를 사용하지 않는다.
		//기본 생성자를 정의 하지 않고 매개 변수가 있는 생성자를 정의하게 되면
		//기존 제공되던 기본 생성자를 사용 할 수 없고 따로 정의를 해줘야 한다.
		//매개 변수가 있는 생성자를 정의할 때는 기본 생성자도 함께 정의해줘야 한다.
		...
		//기본생성자에서 속성 값을 초기화 하지 않을 경우 int는 0으로 초기화
		//String은 null로 초기화
		System.out.println("학번은 : " + student1.getsNo());
		System.out.println("학생이름은 : " + student1.getsName());
		...
		System.out.println("학생이름은 : " + student2.getsName());
