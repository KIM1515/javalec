...
public class NumBaseball {
	...
	public void start() {
		...
		//중복 처리 방식 : 0번째 인자 값은 검사x, 1번째 인자 값부터 검사
		...
		while(true) {
			//사용자가 다시 입력하기 전에 strike, ball 0으로 초기화
			//초기화 안하면 이전 입력된 값의 strike, ball의 값을 가지고 있음
			strike = 0;
			ball = 0;
			...
			//judgeStrike 메소드에서 strike가 3 이상일 경우
			//true를 리턴하기 때문에 if문의 메소드가 true일 때는 무조건 strike가 3이상이다=>게임종료
		...
		public void generateRandom() {
			for(int i = 0; i < com.length; i++) {
				...
				//중복체크
				//checkDupicate()메소드 결과가 true 일 때 
		...
		public boolean checkDuplicate(/*int flag*/int arr[], int num) {
			...
			//중복이 안된 경우
			//for문안의 if문을 타지 않으면 결국 for문 밖으로 나오게 된다.
			return false;
			
		}

		//스트라이크/볼 판정, 스트라이크/볼 출력,
		//스트라이크 개수로 계속 입력인지 게임 종료인지 판단해주는 메소드
		//(리턴 값 boolean으로 true, false) strike < 3이면 false; else return true;
		//judgeStrike
		public boolean judgeStrike() {
			...
			if(strike < 3) {
				...
				System.out.println(gameCnt + "회만의 맞추셨습니다.");
				return true;
			}
