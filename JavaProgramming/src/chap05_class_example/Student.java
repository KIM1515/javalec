...
public class Student {
	...
	//다른 객체를 속성(멤버 변수)로 선언 할 수 있다.
	//하나의 클래스에서 다른 클래스를 참조하여 사용할 때 이것을 의존성이라고 표현한다.
	//Course의 내용이 변경되거나 속성이 바뀌면 Student도 변경 될 수 밖에 없는데
	//이 때 Student가 Course에 의존되어 있으므로 의존성이라고 부른다.
	//좋은 프로그래밍은 결합도가 높고 의존도가 낮아야된다.
	//의존도를 낮추는 방법은 클래스 간의 참조관계를 적게 사용하는 것이다.
	
	private Course course; //수강과목
	
	//기본생성자 정의 안함
	
	//매개 변수 있는 생성자(기본 생성자가 없는 상태)
	//기본 생성자 is undefined 에러를 방지하기 위해 기본 생성자 정의
	public Student() {}
